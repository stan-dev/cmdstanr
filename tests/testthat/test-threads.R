...
test_that("stan_threads works with FALSE and warns on non-logical values", {
  skip_on_cran()
  ...
  expect_warning(
    ...
    "'stan_threads' was assigned a non-logical value. Please use 'TRUE' or 'FALSE'.",
    fixed = TRUE
  )
  mod <- cmdstan_model(stan_program, cpp_options = list(stan_threads = "dummy string"), compile = FALSE)
  expect_warning(
  ...
  expect_warning(
    mod$compile(cpp_options = list(stan_threads = "dummy string"), force_recompile = TRUE),
    "'stan_threads' was assigned a non-logical value. Please use 'TRUE' or 'FALSE'.",
    fixed = TRUE
  )
  expect_output(
