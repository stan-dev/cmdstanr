...
test_that("output() method works after gq", {
  ...
  if (fit_gq$num_procs() == fit_gq$num_chains()) {
...
test_that("time() works after gq", {
  ...
  expect_true(nrow(run_times$chains) %in% c(0L, fit_gq$num_chains()))
  ...
  if (!length(run_times$chains$total)) {
    expect_length(run_times$chains$total, 0L)
  } else if (any(run_times$chains$total > 0)) {
    # Some CmdStan 2.38 development builds and CmdStan >= 2.39 report
    # generated-quantities timing.
    expect_gt(min(run_times$chains$total), 0)
  } else {
    # Earlier CmdStan builds do not report generated-quantities timing.
...
test_that("time() aligns successful gq process ids and times", {
  ...
  runset <- CmdStanRun$new(fit_gq$runset$args, MockGQProcs$new())
