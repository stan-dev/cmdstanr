data {
  tuple(int, vector[2]) d_pair;
  // not tuple(real, complex): CmdStan 2.40's JSON reader misreads a
  // complex value inside an array of tuples
  array[2] tuple(real, real) d_arr;
  complex_vector[2] d_zv;
}
parameters {
  real a_scalar;
...
generated quantities {
  ...
  real d_sum = d_pair.1 + sum(d_pair.2) + d_arr[2].2 + get_imag(d_zv[1]);
}
