...
across multiple chains are not automatically aggregated). Details on the
column names are available in the
[CmdStan documentation](https://mc-stan.org/docs/cmdstan-guide/stan_csv_apdx.html#profiling-csv-output-file).

```{r profiles}
fit$profiles()
```

The `total_time`, `forward_time`, and `reverse_time` columns are measured in
seconds.
...
allocated on the chaining and non-chaining autodiff stacks. These counts can
serve as rough proxies for memory use, but they are not measurements in bytes.
...
Profiling files are scoped to physical CmdStan invocations. In particular, a
threaded executable running several logical chains internally produces one
aggregate profile file for that invocation.
