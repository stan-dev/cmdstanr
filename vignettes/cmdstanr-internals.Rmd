...
CmdStanR writes a hidden JSON file next to the executable that records how the
executable was built: the Stan file and the files it includes, the options
passed to `stanc` and `make`, the CmdStan installation, and a hash of the
executable itself. The file is named after the executable, so `bernoulli` is
described by `.bernoulli.cmdstanr.json`.
