...
To turn on pedantic mode, set `pedantic = TRUE` in the call to
`cmdstan_model()`.
...
Starting with v1.0, CmdStanR writes a hidden JSON file next to the executable so
it can tell whether a rebuild is needed. The file records how the executable was
built: the Stan file and the files it includes, the options passed to stanc and
make, the CmdStan installation, and a hash of the executable itself. The file is
named after the executable, so `bernoulli` is described by
...
# demonstrating stan_build_info even though we can use mod$build_info()
# since we have the model object available
...
If you move the executable, move the build record with it.
...
[CRAN source packages may not contain binary executable code](https://cran.r-project.org/web/packages/policies.html).
