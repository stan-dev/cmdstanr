...
just the Stan file and then calls its [`$compile()`](https://mc-stan.org/cmdstanr/reference/model-method-compile.html)
...
the `$variables()` method of the `CmdStanModel` object. This method works whether
or not the model has been compiled. Here we use `compile = FALSE` because
