...
provide additional details about compiling models, allowed formats for passing
in data, and how CmdStan output is saved and read back into R.

We will use only the `$sample()` method in examples, but all model fitting
methods work the in a similar way under the hood.
...
```{r compile-options}
mod <- cmdstan_model(stan_file, force_recompile = TRUE, cpp_options = list(stan_threads = TRUE))
```
...
For models with many parameters, if only some parameters are requested (e.g., by
specifying the `variables` argument to `fit$draws()`) then CmdStanR will only
read in the requested variables (unless they have already been read in).

### read_sample_csv()

Internally, the `read_sample_csv()` function is used to read the CmdStan CSV
files into R. This function is exposed to users, so you can also call it
directly:

```{r read_sample_csv}
# see ?read_sample_csv for info on optional arguments controlling 
# what information is read in
csv_contents <- read_sample_csv(fit$output_files())
str(csv_contents)
```
