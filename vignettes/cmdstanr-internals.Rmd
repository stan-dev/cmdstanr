...
```{r compile}
# Copy the example to a tempfile so the vignette doesn't modify 
...
`stan_file_variables()` on the file itself when there is no need to compile the
...
")
variables <- stan_file_variables(variables_stan_file)
```
...
```{r build-record}
list.files(dirname(mod$exe_file()), pattern = "cmdstanr.json", all.files = TRUE)
```
