...
The `$diagnose_sampler()` method will display any sampler diagnostic warnings and return a summary of diagnostics for each chain. 
...
```
After fitting there is a warning about divergences. We can also regenerate this warning message later using `fit$diagnose_sampler()`. 

```{r diagnose_sampler-with-warnings}
diagnostics <- fit_with_warning$diagnose_sampler()
print(diagnostics)
