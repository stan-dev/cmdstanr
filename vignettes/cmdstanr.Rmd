...
**CmdStanR is not on CRAN yet and user facing functions are still subject to change**, 
but the latest pre-beta release can be installed via:
...
# load posterior package and convert to matrix (or data frame)
# with all chains merged
library(posterior)
draws_matrix <- as_draws_matrix(draws_array) # as_draws_df() for data frame
print(draws_matrix)
```
