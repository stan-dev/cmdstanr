...
calling the [`install_cmdstan()`](https://mc-stan.org/cmdstanr/reference/install_cmdstan.html)
function:

```{r install_cmdstan-1, include = FALSE}
if (is.null(cmdstan_default_path())) {
  install_cmdstan()
}
```
...
1. If the environment variable `"CMDSTAN"` points directly to a valid CmdStan
installation at load time, that path is used for the R session. If it instead
points to an existing parent directory containing versioned CmdStan
installations, the installation with the largest version number is used. This
...
`".cmdstan/cmdstan-[version]"`, for example `".cmdstan/cmdstan-2.38.0"`,
...
CmdStanR derives chain execution from the compiled executable. If the model was
compiled with `cpp_options = list(stan_threads = TRUE)`, all four logical chains
above run in one CmdStan invocation and share one thread pool. Otherwise
CmdStanR launches one process per chain, with at most `parallel_chains` active at
once. The preferred direct control for the shared pool is `threads`:

```{r threaded-sample, eval=FALSE}
mod_threaded <- cmdstan_model(
  "model.stan",
...
`threads_per_chain` remains available as a sizing heuristic when `threads` is
not supplied. It does not reserve threads for individual chains. For threaded
multi-chain fits, `fit$num_chains()` and chain CSV methods remain chain-scoped,
while `fit$num_procs()`, `fit$return_codes()`, raw console output, profiles, and
saved configurations are invocation-scoped.

There are many more arguments that can be passed to the `$sample()` method. For
example, initial values can be specified with the `init` argument, including by
passing a fitted model object produced by any of CmdStanR's fitting methods.
...
`$cmdstan_summary()` method. This method prints summaries produced by CmdStan
but, unlike `$summary()`, does not return them as a data frame for further use
in R.
...
```
After fitting there is a warning about divergences. We can also regenerate this warning message later using `fit_with_warning$diagnostic_summary()`.
...
the `$cmdstan_diagnose()` method. This method prints CmdStan's diagnostic output
but does not return it as a structured R object.
...
Following CmdStan's terminology, we can find the (penalized) maximum likelihood
estimate (MLE) using [`$optimize()`](https://mc-stan.org/cmdstanr/reference/model-method-optimize.html).
...
adjustment for constrained variables. Including the Jacobian adjustment by setting `jacobian=TRUE` produces a maximum a posteriori (MAP) estimate. More precisely, the default
finds a mode of the target in the constrained parameter space, whereas
`jacobian=TRUE` finds a mode of the corresponding density in the unconstrained
space. See the
...
distribution in the unconstrained space. Following CmdStan's terminology, if
the mode is a MAP estimate, the draws provide an estimate of the mean and
standard deviation of the posterior distribution. If the mode is an MLE, the
draws provide an estimate of the standard error of the likelihood. Whether
the mode is called MAP or MLE depends on the value of the `jacobian` argument
when running optimization. The Laplace `jacobian` setting must match the setting
used for optimization so that both use the same target density. See the
...
Stan version 2.33 introduced the Pathfinder variational inference method.
For details on how Pathfinder works see the section in the
...
The [`$generate_quantities()`](https://mc-stan.org/cmdstanr/reference/model-method-generate-quantities.html)
method runs a Stan program's `generated quantities` block using parameter values
from a previous fit, without rerunning inference. It accepts fitted model
objects produced by any of the fitting methods described above. See the method
documentation for details and a complete example.

## Saving fitted model objects

The [`$save_object()`](https://mc-stan.org/cmdstanr/reference/fit-method-save_object.html)
...
There are additional vignettes available that discuss many other aspects of
