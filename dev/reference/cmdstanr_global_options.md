...
- `cmdstanr_no_ver_check`: Should the check for a more recent version of
  ...
  `cmdstanr_no_ver_check` environment variable to `"true"`
  (case-insensitive). Configure the option or environment variable
...
- `cmdstanr_spinner`: Should a spinner be shown while CmdStan compiles a
  model, checks syntax, or is installed or rebuilt? The default is
  `TRUE`. The spinner is only ever shown in interactive sessions, so
  setting this to `FALSE` is only necessary to suppress it in an
  interactive session.
